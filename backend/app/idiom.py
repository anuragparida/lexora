...
# ---------------------------------------------------------------------------
# Read-only Phrase ORM mirror.
#
# Phase 8.1 (card t_d967c006) owns the authoritative SQLAlchemy
# ``Phrase`` model; this local declaration mirrors the expected 8.1
# schema so this module compiles, runs DSPy offline with ``DummyLM``,
# and the per-row queries filter on the same column names the
# production generator (8.4) will use. The join key is
# ``__tablename__ = "phrases"`` — once 8.1 lands on main, this
# mirror becomes ``Phrase = models.Phrase`` (a re-export, the
# Phase 7.2 pattern for ``Collocation = models.Collocation``).
#
...
class Phrase(Base):
    """Phase 8.3 — read-only mirror of the curated ``phrases`` table.

    Mirror of the schema 8.1 ships (``backend/app/models.Phrase``
    once 8.1 lands on main). Column-for-column alignment with the
    Alembic migration 8.1 authors:

    - ``id`` (TEXT, slug, PK) — per Phase 8 spec §"Files affected
      (anticipated)": ``id`` is a slug like ``ins-blaue-hinein``,
      not an autoincrement int.
    - ``phrase`` (TEXT, UNIQUE) — the German idiom verbatim.
    - ``definition`` (TEXT) — the learner-facing definition
      (1..400 chars on the wire side).
    - ``example_usage`` (TEXT, nullable) — optional DWDS example
      sentence.
    - ``source_attribution`` (TEXT) — comma-joined literal
      (``dwds`` / ``goethe`` / ``schiller``).
    - ``frequency_band`` (TEXT) — ``high`` / ``mid`` / ``low``.
    - ``dwds_url`` (TEXT, nullable) — DWDS source URL.
    - ``attested_quote`` (TEXT, nullable) — Goethe / Schiller
      attestation (8.2's seed).
    - ``attested_source`` (TEXT, nullable) — Goethe / Schiller
      work + chapter citation.
    - ``created_at`` (TIMESTAMPTZ) — DB default to ``sa.func.now()``.

    The mirror is declared here so the module compiles before 8.1
    lands on main. When 8.1 lands, this class becomes a thin
    re-export of ``models.Phrase`` — the SQLAlchemy identity check
    is irrelevant for our purposes because all queries filter by
    table name (``__tablename__``) and column name, not Python
    class identity.
    """
    ...
    source_attribution = Column(String, nullable=False)
    frequency_band = Column(String, nullable=False)
    dwds_url = Column(String, nullable=True)
    attested_quote = Column(Text, nullable=True)
    attested_source = Column(String, nullable=True)
    created_at = Column(DateTime, nullable=False, server_default=func.now())


# Re-export so the rest of this module uses ``Phrase`` as if 8.1
# had landed. Mirrors the Phase 7.2 ``Collocation = models.Collocation``
# pattern.
try:
    # If 8.1 is already on main, prefer the canonical model. The
    # SQLAlchemy identity check (``isinstance(row, models.Phrase)``)
    # works either way because the local mirror above is byte-identical
    # to the canonical model after the fold.
    _Canonical = getattr(models, "Phrase", None)
    if _Canonical is not None:
        Phrase = _Canonical  # type: ignore[misc,assignment]
except (ImportError, AttributeError):
    # 8.1 hasn't landed yet — keep the local mirror.
    pass
